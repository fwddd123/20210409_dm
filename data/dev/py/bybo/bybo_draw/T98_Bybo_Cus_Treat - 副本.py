...
Treat_sql_create_detail_tmp= """ create table IF NOT EXISTS T98_Bybo_Cus_Treat_ChDetail_tmp 
 (chargeorderid varchar(10) NOT NULL    DEFAULT ''COMMENT'',
Region  varchar(10) NOT NULL    DEFAULT ''COMMENT'',
...
on a.id = b.chargeorderid and a.Region = b.Region
and b.SourceDetailId > 0
and b.IsInactive = 0 
where a.IsInactive = 0 
and a.STATUS not in('未收费','作废并撤回')
and a.Scenario in (0,6)
and date(a.PayDateTime) between '{self.begin_date}' and '{self.begin_date}'
GROUP BY 1,2,3,4;	 
"""
...
and a.STATUS not in('未收费','作废并撤回') 
and a.Scenario in (0,6)
and date(a.PayDateTime) between '{self.begin_date}' and '{self.end_date}'
...
and A.IsInactive = '0'  
and A.PayType <> '余额调整' 
and a.RecordCreatedDate between '2021-01-01' and '2021-01-02' and
...
where Chrg_Dt>='{self.begin_date}'  and Chrg_Dt<='{self.begin_date}'
"""
Cus_Treat_insert="""
...
Cus_Treat = [Cus_Treat_dtail_tmp_truncate,Cus_Treat_dtail_tmp_insert,Cus_Treat_insert_tmp,Cus_Treat_del,Cus_Treat_insert
